...
public class AlterCalca {
    ...
    public void calca () throws SQLException {
        ...
        System.out.print("Informe o código da calça no qual gostaria de atualizar: ");
        int oldCod = sc.nextInt();

        String sql = "SELECT * FROM calcas WHERE codigo = ?";
        ...
        if (calcas.isEmpty()){
            ...
            for (Calca c : calcas) {
                System.out.println("Gostaria de atualizar o registro da seguinte calça" +
            ...
            while (continuar){
                ...
                if (condition.equalsIgnoreCase("S")) {
                    ...
                    if (desejo == 1) {
                        ...
                        sql = "UPDATE calcas set MARCA = ? WHERE codigo = ?";
                        ...
                        sql = "UPDATE calcas set TAMANHO = ? WHERE codigo = ?";
                        stmt = connection.prepareStatement(sql);
                        ...
                        sql = "UPDATE calcas set COR = ? WHERE codigo = ?";
                        stmt = connection.prepareStatement(sql);
                        ...
                        sql = "UPDATE calcas set MODELO = ? WHERE codigo = ?";
                        stmt = connection.prepareStatement(sql);
                        ...
                        sql = "UPDATE calcas set QUANTIDADE = ? WHERE codigo = ?";
                        stmt = connection.prepareStatement(sql);
